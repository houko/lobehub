...
const authSsgManifestPath = path.resolve(root, 'dist/auth/ssg-manifest.json');

if (existsSync(authHtmlPath)) {
  const authHtml = readFileSync(authHtmlPath, 'utf8');
  const authSsgHtmlTemplates = JSON.parse(readFileSync(authSsgManifestPath, 'utf8'));
...
export const authSsgHtmlTemplates = ${JSON.stringify(authSsgHtmlTemplates)};
`;
